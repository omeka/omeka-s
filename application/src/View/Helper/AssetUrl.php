...
class AssetUrl extends AbstractHelper
{
     ...
     * @param bool $override
     * @param bool $versionned Append the version of Omeka, the theme or the
     * module as a url query to improve cache.
     * @return string|null
     */
    public function __invoke($file, $module = null, $override = false, $versionned = true)
    {
        ...
        if (null === $module && $this->currentTheme) {
            return sprintf(self::THEME_ASSETS_PATH, $basePath, $this->currentTheme->getId(),
                $file, $versionned ? '?v=' . $this->currentTheme->getIni('version') : '');
        }
