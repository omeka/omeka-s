...
class FormCollectionElementGroupsCollapsible extends FormCollectionElementGroups
{
    public function render(ElementInterface $element): string
    {
        ...
        foreach ($elementGroups as $elementGroupName => $elementGroupLabel) {
            ...
            $markup .= sprintf('<button type="button" class="expand" title="%s"><span class="fieldset-label" id="%s-label">%s</span></button>', $view->escapeHtml($view->translate('Expand')), $view->escapeHtml($elementGroupName), $view->escapeHtml($view->translate($elementGroupLabel)));
